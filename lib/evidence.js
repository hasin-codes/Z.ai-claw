...
async function extractEvidence(userMessage, scenario) {
  ...
  const prompt = `You are extracting structured support evidence from a user message.

The user is reporting a "${scenario}" issue. Extract any of the following fields that are present in the message. Return ONLY a JSON object with the fields you found. Use null for fields not present. Do not guess or invent values.

Fields to extract:
${fieldList}

Respond with valid JSON only. Example: {"user_id": "123456", "base_url": "https://api.z.ai", "charge_timestamp": null}`;
